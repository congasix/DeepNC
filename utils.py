...
def deledges(graphG, p):
    ''' Randomly removing edges from a graph
    Args:
        p_del: probability of removing edges
    Returns:
        Subgraph after edge deletion
    '''
    G = graphG.copy()
    edge_remove_count = 0
    for (u, v) in list(G.edges()):
        if np.random.rand()<p:
            G.remove_edge(u, v)
            edge_remove_count += 1
    print("Number of removed edges: ", edge_remove_count)
    return G
...
if __name__ == '__main__':
    ...
    #graphs = load_graph_list('graphs/' + 'GraphRNN_MLP_community4_4_128_pred_100_1.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_community4_4_128_train_0.dat')
    #graphs = load_graph_list('eval_results/mmsb/' + 'community41.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_community4_4_128_test_0.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_citeseer_4_128_test_0.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_protein_4_128_test_0.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_barabasi2000_4_128_test_0.dat')
    graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_facebook_4_128_test_0.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_enzymes_4_128_test_0.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_barabasi_4_128_test_0.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_lfr_4_128_test_0.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_lfr2000_4_128_test_0.dat')
    #graphs2 = load_graph_list('graphs/' + 'GraphRNN_MLP_celegans_4_128_test_0.dat')
    ...
    """
    ## Delete nodes from training graphs
    for frozengraph in graphs2:
        graph = nx.Graph(frozengraph)
        max_node = len(graph)
        while (len(graph) > 0.95*max_node):
            random_node = np.random.choice(graph.nodes())
            randnum = np.random.random_sample()
            if randnum < 1/graph.number_of_nodes():
            #if randnum < 1/10:
                print(random_node)
                print(len(graph))
                graph.remove_node(random_node)
    save_graph_list(graphs2,'graphs/' + 'GraphRNN_MLP_pro95_4_128_test_0.dat')
    
    """
    np.random.seed(0)
    #graphs2[0].remove_node(0)
    G=graphs2[5].copy()
    print(len(G))
    #print("fastPFP.")
    #nx.write_edgelist(G, "ground_truth_face.txt")
    G = nx.read_edgelist("ground_truth_face.txt")
    ## delete 30%, remain 70%
    """
    print(len(G))
    ...
    size = 0.7*max_node
    object4=ForestFire()
    sample4 = object4.forestfire(G,size) # graph, number of nodes to sample
    print("Number of nodes sampled=",len(sample4.nodes()))
    print("Number of edges sampled=",len(sample4.edges()))
    nx.write_edgelist(sample4, "delgraph_face_FF.txt")
    ...
    print(len(G))
    nx.write_edgelist(G, "delgraph_face_RN.txt")
    """
    ## delete 70%, remain 30%
    ...
    print("Number of nodes sampled=",len(sample4.nodes()))
    print("Number of edges sampled=",len(sample4.edges()))
    nx.write_edgelist(sample4, "delgraph_face_FF30.txt")
    
    #max_node = len(G)
    while (len(G) > 0.3*max_node):
        random_node = np.random.choice(G.nodes())
        randnum = np.random.random_sample()
        if randnum < 1/G.number_of_nodes():
        #if randnum < 1/10:
    ...
    print(len(G))
    nx.write_edgelist(G, "delgraph_face_RN30.txt")
