...
def sample_sigmoid_greedy(y, sample, thresh=0.5, sample_time=2):
    '''
        do sampling over unnormalized score
    :param y: input
    :param sample: Bool
...
def sample_sigmoid_supervised_simple_origin(y_pred, y, current, y_len, sample_time=2):
    '''
        do sampling over unnormalized score
    :param y_pred: input
    :param y: supervision
...
def sample_sigmoid_supervised_simple(y_raw_all, y_pred, y, current, y_len, sample_time=2):
    '''
        do sampling over unnormalized score
    :param y_pred: input
    :param y: supervision
    ...
    for i in range(y_result.size(0)):
        # using supervision
        update_values = np.zeros(y_result.size(2))
        
        for j in range(len(y_raw_all[i][current])):
            if y_raw_all[i][current][j] != 100:
                update_values[j] = y_raw_all[i][current][j]
            else:
                update_values[j] = y[i,0,j]
        
        if current<y_len[i]:
            y_result[i] = torch.FloatTensor(update_values)
            #y_result[i] = y[i]
        # supervision done
        else:
            # do 'multi_sample' times sampling
...
def sample_sigmoid_supervised_greedy(y_raw_all, y_pred, current, sample_time=2):
    '''
        do sampling over unnormalized score
    :param y_pred: input
    :param y: supervision
    ...
    for i in range(y_result.size(0)):
        ...
        for j in range(len(y_raw_all)):
            if y_raw_all[j] != 100:
                update_values[j] = y_raw_all[j]
            else:
                for k in range(sample_time):
                    ...
                    if (torch.sum(y_result[i]).data>0).any():
                        break
                update_values[j] = y_result[i][0][j]
...
def sample_sigmoid_supervised_greedy2(obs_graph, reslist, nodeid, y_pred, current, sample_time=2):
    '''
        do sampling over unnormalized score
    :param y_pred: input
    :param y: supervision
...
def imputation(node,obs_graph,nodeid,y_result,reslist):
    j = reslist.index(node)
    if obs_graph.has_edge(nodeid,node):
        if obs_graph[node][nodeid]['weight'] != 100:
            return 1
            #print("co edge")
...
def imputation(node,obs_graph,nodeid,y_result,reslist):
    ...
    if y_result[j] == 1:
        ...
        return 1
"""
def sample_sigmoid_supervised_greedy_lp(obs_graph, reslist, nodeid, y_pred, current, sample_time=2):
    '''
        do sampling over unnormalized score
    :param y_pred: input
    :param y: supervision
    ...
    for i in range(y_result.size(0)):
        ...
        #y_thresh = Variable(torch.rand(y_pred.size(1), y_pred.size(2))).cuda()
            ...
            y_result[i] = torch.gt(y_pred[i], y_thresh).float()
            # thresholding by 0.x
            #y_result[i] = torch.gt(y_pred[i], 0.2).float()
            if (torch.sum(y_result[i]).data>0).any():
                break
        pred = y_pred[i][0].detach().cpu().numpy()
        y_res = y_result[i][0].cpu().numpy()
        #print("predict: ", pred)
        #print("result: ", y_res)
        
        # serial processing
        update_values[:(len(reslist))] = [imputation(node,obs_graph,nodeid,y_res,reslist) for node in reslist]
        # parallel processing
        # Note: running time is slower (for small graphs) but scale better
        """
        with closing(mp.Pool(10)) as p:
            #if reslist == []:
            #    update_values[j] = 1
            
            # synchronous multithread
            update_values[:(len(reslist))] = [p.apply(imputation, (node,obs_graph,nodeid,y_res,reslist)) for node in reslist]
            
            # asynchronous multithread
            #results = [p.apply_async(imputation, (node,obs_graph,nodeid,result[j],j)) for node in reslist]
            #update_values = [p.get() for p in results]
        p.join()
        """
        #    j = j+1
...
class GRU_plain(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, num_layers, has_input=True, has_output=False, output_size=None):
        ...
        if has_input:
            self.input = nn.Linear(int(input_size), embedding_size)
